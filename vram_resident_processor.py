...
# ══════════════════════════════════════════════════════════════════════════════
# ══════════════════════════════════════════════════════════════════════════════
# HARDWARE CONTEXT & PINNING FOR EXCLUSIVITY
# GPU 0: RTX 5080 (16 GB VRAM) → Pins deepseek-r1:14b-qwen-distill-q8_0 (~15 GB)
# GPU 1: RTX 3080 (10 GB VRAM) → Pins qwen2.5-coder:14b (~9 GB)
# This keeps BOTH models resident concurrently in separate VRAM spaces,
# eliminating the costly NVMe load/evict loop (30-60s per model swap).
# ══════════════════════════════════════════════════════════════════════════════
...
MODEL_TIERS = {
    ...
    # ── Single-GPU tier  ─────────────────────────────────────────────────
    "single":       "deepseek-r1:14b",
    "single_code":  "qwen2.5-coder:14b",
    # ── Fast fallback  ───────────────────────────────────────────────────
    "fast":         "deepseek-r1:8b",
}
...
def _ollama_get_loaded(url: str) -> List[str]:
...
def _ollama_evict(model: str, url: str) -> bool:
...
def _ollama_wait_clean(url: str, timeout: int = 30, interval: float = 2.0) -> bool:
    """Poll /api/ps until no models are loaded or timeout expires."""
    deadline = time.time() + timeout
    while time.time() < deadline:
        if not _ollama_get_loaded(url):
            return True
        time.sleep(interval)
    return False


def _ollama_restart_service(url: str) -> bool:
    """Restart the ollama systemd service and wait for it to come back online."""
    print(f"  🔄  Restarting ollama service at {url} …")
    try:
        r = subprocess.run(
...
def provision_ollama(url: str, verbose: bool = False) -> bool:
    """
    ...
    """
    print(f"\n  🎯  Override: provisioning Ollama ({url}) for exclusive GPU use …")

    loaded = _ollama_get_loaded(url)
    if not loaded:
        print(f"  ✓   No models currently loaded at {url} — GPU is free")
        return True

    # Level 1: graceful eviction via keep_alive=0
    print(f"  ⚡  Evicting {len(loaded)} loaded model(s) at {url}: {', '.join(loaded)}")
    for model in loaded:
        print(f"       → {model} …", end="", flush=True)
        ok = _ollama_evict(model, url)
        print(" ✓" if ok else " ✗")

    if _ollama_wait_clean(url, timeout=20):
        print(f"  ✅  All models evicted at {url} — GPU is free\n")
        return True

    # Level 2: force a service restart
    remaining = _ollama_get_loaded(url)
    print(f"  ⚠️   {len(remaining)} model(s) still loaded at {url} — escalating to service restart")
    if not _ollama_restart_service(url):
        print("  ⚠️   Service restart failed — proceeding anyway\n")
        return False

    if _ollama_wait_clean(url, timeout=15):
        print(f"  ✅  Ollama at {url} restarted clean\n")
        return True

    print(f"  ⚠️   Could not confirm clean VRAM at {url} — proceeding anyway\n")
    return False
...
# database (paper_store.py), not in per-paper output folders.
# ══════════════════════════════════════════════════════════════════════════════
...
class Backend:
    ...
    def __init__(self, name: str, default_model: str, url: str = PRIMARY_OLLAMA_URL):
        self.name          = name
        ...
        for attempt in range(1, 3):
            ...
            try:
                ...
                if attempt == 1 and not _shutdown.is_set():
                    ...
                    _ollama_restart_service(self.url)
                    continue
...
class PaperProcessor:
    ...
    def process(self, pdf_path: Path) -> None:
        ...
        try:
            ...
            try:
                context = map_reduce_chunks(chunks, self.primary_backend, model)
            ...
            if _should_run("summary"):
                ...
                print("     📝  Summary …")
                try:
                    out = self.primary_backend.call(self._tag_prompt(PROMPTS["summary"], capped), model)
                except _ShutdownRequested:
            ...
            if _should_run("logic"):
                ...
                try:
                    out = self.primary_backend.call(self._tag_prompt(PROMPTS["logic"], capped), model)
                except _ShutdownRequested:
                    print(f"     ⚡  Interrupted mid-section — {len(completed)} section(s) saved")
                    return
            ...
            if _should_run("cpp"):
                ...
                try:
                    out = self.code_backend.call(self._tag_prompt(PROMPTS["cpp"], capped), code_model)
                except _ShutdownRequested:
                    print(f"     ⚡  Interrupted mid-section — {len(completed)} section(s) saved")
                    return
                paper_store.write_section(
            ...
            if _should_run("diagrams"):
                ...
                if not diagrams:
                    # Save raw output so the user can inspect / manually
                    # extract DOT. Existing diagrams (if any, from a prior
                    # successful run) are left untouched — only a real
                    # reparse purges them.
            ...
            if _should_run("extras"):
                ...
                try:
                    out = self.primary_backend.call(self._tag_prompt(PROMPTS["extras"], capped), model)
                except _ShutdownRequested:
                    print(f"     ⚡  Interrupted mid-section — {len(completed)} section(s) saved")
                    return
                paper_store.write_section(
...
def main():
    ap = argparse.ArgumentParser(
        ...
        epilog=textwrap.dedent("""\
            ...
            Model auto-selection by page count (use -s to pick interactively):
              ...
              > 18 pages  →  deepseek-r1:14b-qwen-distill-q8_0  (~15 GB, primary pinned)
              C++ section →  qwen2.5-coder:14b                  (~9 GB, code pinned)
    ...
    ap.add_argument(
        "--primary-url", default=None, metavar="URL",
        help="Ollama backend URL for primary model (default: OLLAMA_URL or http://localhost:11434)",
    )
    ap.add_argument(
    ...
    # Set up global Ollama URL overrides
    global PRIMARY_OLLAMA_URL, CODE_OLLAMA_URL
    if args.primary_url:
        PRIMARY_OLLAMA_URL = args.primary_url
    if args.code_url:
        CODE_OLLAMA_URL = args.code_url
    ...
    # ── Health check ───────────────────────────────────────────────────────
    print(f"\n  🦞  OpenClaw Paper Processor (VRAM Concurrent Resident)")
    print(f"  {'─'*55}")
    ok = True
    if args.backend == "ollama":
        ok = health_check_ollama(PRIMARY_OLLAMA_URL, "Primary")
        if PRIMARY_OLLAMA_URL != CODE_OLLAMA_URL:
            ok = ok and health_check_ollama(CODE_OLLAMA_URL, "Code")
    else:
        ok = health_check_openclaw()
    if not ok:
        sys.exit(1)

    if args.backend == "ollama":
        primary_models = [args.model] if args.model else list(dict.fromkeys(
            [MODEL_TIERS[k] for _, k in TIER_BY_PAGES]
        ))
        code_models = [args.code_model] if args.code_model else [CODE_MODEL]
        
        check_required_models(primary_models, PRIMARY_OLLAMA_URL)
        check_required_models(code_models, CODE_OLLAMA_URL)

    if args.override and args.backend == "ollama":
        provision_ollama(PRIMARY_OLLAMA_URL, verbose=args.verbose)
        if PRIMARY_OLLAMA_URL != CODE_OLLAMA_URL:
            provision_ollama(CODE_OLLAMA_URL, verbose=args.verbose)
    ...
    primary_backend = Backend(args.backend, default_model, PRIMARY_OLLAMA_URL)
    
    default_code_model = args.code_model or CODE_MODEL
    code_backend = Backend(args.backend, default_code_model, CODE_OLLAMA_URL)

    print(f"  Backend      : {args.backend}")
    print(f"  Database     : {db_path}")
    print(f"  Primary URL  : {PRIMARY_OLLAMA_URL}")
    print(f"  Code URL     : {CODE_OLLAMA_URL}")
    print(f"  Primary Model: {default_model}")
    print(f"  Code Model   : {default_code_model}")
    if args.reprocess:
