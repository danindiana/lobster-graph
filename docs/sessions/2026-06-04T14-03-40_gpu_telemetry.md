...
## Part 7 — Issue Resolution & Model Discovery Fix (2026-06-04)

### Problem:
When the CLI wizard launched the isolated backend servers via `start_isolated_backends.sh` under user-space `jeb`, it ran `ollama serve` which defaulted to `~/.ollama/models`. This caused the pipeline to abort since the 37 models downloaded by the systemwide Ollama systemd service (which runs as the `ollama` user with model store at `/usr/share/ollama/.ollama/models`) were not visible to the user-space instances.

### Resolution:
1. Identified that `/usr/share/ollama/.ollama/models` is a symlink to `/mnt/samsung_ssd/ollama/.ollama/models` which is world-readable, and the parent directories are group-traversable.
2. Verified that the `jeb` user is a member of the `ollama` group and can access `/usr/share/ollama/.ollama/models` without sudo permissions.
3. Updated [start_isolated_backends.sh](file:///home/jeb/programs/python_programs/paper_processor/docs/sessions/2026-06-04T14-03-40_gpu_telemetry/start_isolated_backends.sh) to export `OLLAMA_MODELS="${OLLAMA_MODELS:-/usr/share/ollama/.ollama/models}"` prior to launching the daemons.
4. Verified that both ports (11434 and 11435) now successfully discover all 37 system models without triggering timeout/auto-download loops.

---

## Part 8 — Concurrency, GPU Interleaving, & Live Token Window

### 1. GPU VRAM Memory Overflows & Dynamic Context Sizing
* **Problem**: Even with the 14B model, requesting a static 32,768 context length forced Ollama to allocate a massive 6 GB KV Cache, which overflowed the RTX 5080's VRAM and triggered layer splitting to the CPU.
* **Solution**: Updated `vram_resident_processor.py` to calculate the context length dynamically (`min(16384, max(4096, len(prompt) // 4 + 2048)`), and reduced explicit context limits in `map_reduce_chunks` to `8192` and diagram generation to `16384`. This shrunk the KV Cache to only ~800 MB - 1.5 GB, keeping the entire `deepseek-r1:14b` model 100% warm in GPU 0 VRAM.

### 2. Multi-GPU Parallel Task Interleaving
* **Concept**: Utilizing both the RTX 5080 (GPU 0) and the RTX 3080 (GPU 1) concurrently for maximum throughput.
* **Mechanism**: By setting `--workers 2` (via command-line or the CLI Wizard), multiple papers are processed concurrently in separate worker threads. Since reasoning is routed to GPU 0 and coding is routed to GPU 1, the pipeline naturally interleaves tasks. For example, Worker A runs code example generation on GPU 1 while Worker B runs summaries on GPU 0, utilizing both cards concurrently for up to a 2x throughput gain.

### 3. Live Token Streaming Terminal Window
* **Feature**: Spawns a dedicated terminal instance showing the live token output as it's generated by the LLM.
* **Implementation**:
  - Modified `_call_ollama` to stream each generated token directly into a shared log file at `/tmp/ollama_tokens.log` in real-time.
  - Spawns a detached `alacritty` terminal window running `tail -f /tmp/ollama_tokens.log` at script startup. This window automatically presents the operator with a live, character-by-character scrolling feed of the reasoning and coding output.
