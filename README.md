...
* **`neo4j_viz/cosmos_*` (CosmosGL Dashboard)**: A second, complementary Neo4j visualization built on [`@cosmos.gl/graph`](https://github.com/cosmosgl/graph), served on port **8686** alongside the main dashboard on 8585. Both read the same live `paper-processor-neo4j` instance. Renders points/links from GPU-precomputed ForceAtlas2 positions (`compute_layout.py`) with client-side simulation disabled, so the full graph — currently ~15K nodes / ~1.2M edges — stays responsive to pan/zoom/hover without running physics in the browser. **Double-click any node** to open a detail panel (definition/statement/pseudocode/code/etc., pulled straight from Neo4j); double-clicking a `Paper` node also tries to open its source PDF in a new tab (`pdf_path`, looked up server-side by node id — if the source file has since been moved or archived, it 404s with an explanation instead of failing silently, and the detail panel still shows the paper's stored summary regardless). Runs as a **systemd service** (`cosmos-dashboard.service`, `Restart=always`), with its up/down state broadcast via `wall`, shown at login via `/etc/update-motd.d/96-cosmosgl-status`, and surfaced by `vram_wizard.py` / `paper_processor_dir.py` at startup.
...
### 9. CosmosGL Dashboard: systemd, wall, motd, and entry-point notices
...
Semantic search + a knowledge graph over a folder of dated session logs (originally built here as `session_archive/`, sharing this repo's Neo4j graph) has been extracted into its own standalone, containerized project: **[`sessionarchive`](https://github.com/danindiana/sessionarchive)**. It bundles its own Neo4j (trading the automatic `Concept` cross-linking with this repo's papers for full portability), packages `ingest`/`query`/`label` as one Docker image with a single CLI, and includes `--like SLUG` ("more like this" similarity) and `--rerank` (relevance-probe re-ranking, a pattern ported from the sibling `militia-classifier` project). See that repo for setup, usage, and architecture docs.
